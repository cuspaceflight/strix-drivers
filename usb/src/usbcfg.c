 ...
 * Endpoints.
 */
#define USB_INTERRUPT_REQUEST_EP        1
...
#define USB_CDC_CIF                     0
#define USB_CDC_DIF                     1
...
static const uint8_t vcom_configuration_descriptor_data[] = {
  ...
  USB_DESC_CONFIGURATION(
    ...
    IAD_CDC_IF_DESC_SET_SIZE,               /* wTotalLength.                */
  ...
  IAD_CDC_IF_DESC_SET(
    USB_CDC_CIF,
    USB_CDC_DIF,
    USB_ENDPOINT_IN(USB_INTERRUPT_REQUEST_EP),
    USB_ENDPOINT_OUT(USB_DATA_AVAILABLE_EP),
    USB_ENDPOINT_IN(USB_DATA_REQUEST_EP)
  ),
};
  ...
  case USB_EVENT_CONFIGURED:
    ...
    if (usbp->state == USB_ACTIVE) {
      /* Enables the endpoints specified into the configuration.
         ...
         must be used.*/
      usbInitEndpointI(usbp, USB_INTERRUPT_REQUEST_EP, &ep1config);
      usbInitEndpointI(usbp, USB_DATA_REQUEST_EP, &ep2config);

      /* Resetting the state of the CDC subsystem.*/
      sduConfigureHookI(&usb_driver);
    }
